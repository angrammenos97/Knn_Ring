    ...
    for (int l = 0; l < k; l++ ){
      ...
      if (ap == COLMAJOR){
        ...
        if ( fabs( knnresndist_cm(j,l) - distxy ) > 1e-6 ) {
			printf("Error in dist: got %e instead of %e. ", knnresndist_cm(j, l), distxy);
			return 0;
		}
        /* distances should be non-decreasing */
        if ( knnresndist_cm(j,l) < maxDist ) {
			printf("Error in order: got %lf while max is %lf. ", knnresndist_cm(j, l), maxDist);
			return 0;
		}
        ...
        if ( fabs( knnresndist_rm(j,l) - distxy ) > 1e-6 ) {
			printf("Error in dist: got %e instead of %e. ", knnresndist_rm(j, l), distxy);
			return 0;
		}
        /* distances should be non-decreasing */
        if ( knnresndist_rm(j,l) < maxDist ) {
			printf("Error in order: got %lf while max is %lf. ", knnresndist_rm(j, l), maxDist);
			return 0;
		}
